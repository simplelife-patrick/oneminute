...
@implementation DLYRecordEncoder
...
//DLYRecordEncoder遍历构造器的
+ (DLYRecordEncoder*)encoderForPath:(NSString*) path Height:(NSInteger) cy width:(NSInteger) cx channels: (int) ch samples:(Float64) rate {
    DLYRecordEncoder* enc = [DLYRecordEncoder alloc];
...
- (BOOL)encodeFrame:(CMSampleBufferRef) sampleBuffer isVideo:(BOOL)isVideo {
    ...
    if (CMSampleBufferDataIsReady(sampleBuffer)) {
        //写入状态为未知tracksWithMediaType,保证视频先写入
